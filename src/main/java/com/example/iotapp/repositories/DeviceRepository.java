...
public interface DeviceRepository extends JpaRepository<Device, Long> {
    ...
    Device getDeviceBySerialNumber(@Param("serialNumber") String serialNumber);
    ...
    @Query(nativeQuery = true,
            value = "SELECT * FROM devices WHERE date(date_of_add) = date(CONCAT(:date, '%'))",
            countQuery = "SELECT count(*) FROM devices WHERE date(date_of_add) = date(CONCAT(:date, '%'))")
